...
export const AuthModal: React.FC = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (email) {
      signInWithEmail(email, fullName);
    }
  };

  const handleGoogleAuth = () => {
    signInWithGoogle(fullName || undefined);
  };
  ...
  return (
    ...
    >
      <div 
        ...
        className="w-full max-w-md rounded-3xl border border-white/10 bg-[#0c101c] shadow-2xl p-6 sm:p-8 relative space-y-6 my-auto"
      >
        ...
        <div className="flex items-center gap-3 text-slate-600">
          ...
          <span className="text-[10px] font-fira uppercase">or enter details</span>
          <div className="flex-1 h-px bg-white/10" />
        </div>

        {/* Personalized Name & Email Form */}
        <form onSubmit={handleSubmit} className="space-y-3">
          <div className="relative">
            <User className="w-4 h-4 text-slate-500 absolute left-3.5 top-3.5" />
            <input
              type="text"
              placeholder="Your Full Name (e.g. Sarthak, Priya, Alex)"
              value={fullName}
